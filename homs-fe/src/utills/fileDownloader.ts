...
function getFilenameFromHeaders(
  ...
  const contentDisposition: string | undefined =
    headers["content-disposition"]?.toString();
  ...
  const filenameStarMatch: RegExpExecArray | null =
    /filename\*=(?:UTF-8'')?([^;]+)/i.exec(contentDisposition); // 매치 결과 타입
  if (filenameStarMatch && filenameStarMatch[1]) {
    try {
      ...
      // 에러 객체 타입 명시
      console.warn(
        "UTF-8 filename decoding failed, trying simple filename.",
        e
      ); // 에러 객체도 함께 로깅
    }
  } else {
    // filename="..." 또는 filename=... 형식 처리
    const filenameMatch: RegExpExecArray | null =
      /filename="([^"]+)"|filename=([^;]+)/i.exec(contentDisposition); // 매치 결과 타입
    if (filenameMatch && (filenameMatch[1] || filenameMatch[2])) {
      try {
        ...
        // 에러 객체 타입 명시
        console.warn(
          "Simple filename decoding failed, using default filename.",
          e
        ); // 에러 객체도 함께 로깅
      }
    }
